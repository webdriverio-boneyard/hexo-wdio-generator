...
class Build {
    ...
    async clean () {
        ...
        await this.execute('npm uninstall wddoc && rm -r ./node_modules/webdriverio')
    }
     ...
     */
    async npmInstall (pkg, v) {
        await this.execute('cd ./node_modules')

        /**
         * grab latest version if not specified
         */
        if (!v || v === 'latest') {
            v = await this.execute(`npm show ${pkg} version`)
        }

        let archive = `${pkg}-${v}.tgz`
        await this.execute(`curl --silent --remote-name "https://registry.npmjs.org/${pkg}/-/${archive}"`)
        await this.execute(`mkdir ${pkg}`)
        await this.execute(`tar xzf ./${archive} --strip-components 1 -C "${pkg}"`)
        await this.execute(`rm ./${archive}`)

        await this.execute('cd ../')
    }
    ...
    async build (v, clean) {
        ...
        for (let build of versionsToGenerate) {
             ...
             */
            await this.npmInstall('webdriverio', build.webdriverio)
